...
public final class CYaHPConverter {
	...
	throws CConvertException {
		...
		try {
			Thread.currentThread()
					  .setContextClassLoader(new URLClassLoader(new URL[0],CClassLoader.getLoader(
						"/main")));
	...
	private final void init() {
		...
		config.addFile("/main", loader.getResource("core-renderer.jar"));
		config.addFile("/main", loader.getResource("jaxen-1.1.1.jar"));
	...
	private IHtmlToPdfTransformer getTransformer(final Map properties) {
		...
		if (transformers.containsKey(rendererClassName)) {
			...
			ClassLoader bootStrap = CClassLoader.getLoader("/main");
