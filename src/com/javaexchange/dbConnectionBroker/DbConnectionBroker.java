...
public class DbConnectionBroker implements Runnable {
	...
	public DbConnectionBroker(String dbDriver,
										...
										String logFileString,
										double maxConnTime,
										boolean logAppend)throws IOException {
	...
	private void setupBroker(String dbDriver, String dbServer, String dbLogin,
		...
		try {
			log = new PrintWriter(
					new FileOutputStream(logFileString, logAppend), true);
					...
					synchronized (this) { // Add new connections to the pool
						if (currConnections < maxConns) {
							try {
								...
								if (debugLevel > 0) {
									log
											.println("Error: Unable to create new connection: "
													+ e);
								}
							}
						}
	...
	public String freeConnection(Connection conn) {
		return freeConnectionX(conn,"","");
	}

	public String freeConnection(Connection conn,String page) {
		return freeConnectionX(conn,page,"");
	}

	public String freeConnection(Connection conn,String page,String user) {
		return freeConnectionX(conn,page,user);
	}

	public String freeConnectionX(Connection conn,String page,String user) {
	...
	public void destroy(int millis) throws SQLException {
		...
		try {
			DriverManager.deregisterDriver(DriverManager.getDriver(dbServer));
			System.out.println("Driver unregistered");
		} catch (Exception e) {
			log.println("Error creating connection: " + e);
		}
