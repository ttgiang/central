...
public class StmtServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)throws IOException, ServletException {
		...
		try {
			...
			String statement = website.getRequestParameter(request, "statement");
			...
			if (sAction.equalsIgnoreCase("submit")) {
				iAction = updateAction;
			}

			Stmt stmt = new Stmt(id, type, statement, campus, auditby,
			...
			case cancelAction:
				...
				break;
			case deleteAction:
				rowsAffected = StmtDB.deleteStmt(connection, id);

				if (rowsAffected == 1)
					message = "General statement <b>" + type
							+ "</b> deleted successfully";
				else
					message = "Unable to delete General statement <b>" + type
							+ "</b>";

				break;
			case insertAction:
				rowsAffected = StmtDB.insertStmt(connection, stmt);

				if (rowsAffected == 1)
					message = "General statement <b>" + type
							+ "</b> inserted successfully";
				else
					message = "Unable to insert General statement <b>" + type
							+ "</b>";

				break;
			case updateAction:
				rowsAffected = StmtDB.updateStmt(connection, stmt);

				if (rowsAffected == 1)
					message = "General statement <b>" + type
							+ "</b> updated successfully";
				else
					message = "Unable to update General statement <b>" + type
							+ "</b>";
		...
		if (rowsAffected == -1) {
			...
			url = response.encodeURL("/core/msg.jsp?rtn=stmtidx");
		}
