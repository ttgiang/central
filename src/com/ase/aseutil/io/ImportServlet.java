	...
	**/
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (o == null) {
			...
			if (tp.equals("prereq") || tp.equals("coreq"))
				requisites(request,response,talin,tp,key);
			else if (tp.equals("xlist"))
	...
	private void xlist(HttpServletRequest request,
		...
		try{
			...
			//String message = ImportDB.crossListed(conn,campus,user,key);

			String message = ImportDB.importData(conn,campus,user,key,"xlist");
	...
	private void requisites(HttpServletRequest request,
		...
		*/
		...
		try{
			...
			if (tp.equals("coreq")){
				//message = ImportDB.requisites(conn,campus,user,Constant.REQUISITES_COREQ,key);
				message = ImportDB.importData(conn,campus,user,key,"coreq");
			}
			else if (tp.equals("prereq")){
				//message = ImportDB.requisites(conn,campus,user,Constant.REQUISITES_PREREQ,key);
				message = ImportDB.importData(conn,campus,user,key,"prereq");
			}
			else
				message = "";
