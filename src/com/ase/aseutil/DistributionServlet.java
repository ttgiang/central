...
public class DistributionServlet extends HttpServlet {
	...
	static Logger logger = Logger.getLogger(DistributionServlet.class.getName());
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			logger.info("--------------------------- DistributionServlet - START");
			...
			distribution.setListID(lid);
			listName = website.getRequestParameter(request,"listName","");
			distribution.setTitle(listName);
			distribution.setMembers(website.getRequestParameter(request,"formSelect",""));
			distribution.setCampus(campus);
			distribution.setAuditBy(auditby);
			distribution.setAuditDate(AseUtil.getCurrentDateTimeString());
			...
			switch (iAction) {
				case cancelAction:
					...
					break;
				case deleteAction:
					rowsAffected = DistributionDB.deleteList(connection, lid);
					if (rowsAffected == 1)
						message = "Distribution list <b>" + listName + "</b> deleted successfully";
					else
						message = "Unable to delete distribution list <b>" + listName + "<br/>";

					break;
				case insertAction:
					rowsAffected = DistributionDB.insertList(connection,distribution);
					if (rowsAffected == 1)
						message = "Distribution list <b>" + listName + "</b> inserted successfully";
					else
						message = "Unable to insert distribution list <b>" + listName + "<br/>";

					break;
				case updateAction:
					rowsAffected = DistributionDB.updateList(connection,distribution);
					if (rowsAffected == 1)
						message = "Distribution list <b>" + listName + "</b> updated successfully";
					else
						message = "Unable to update distribution list <b>" + listName + "<br/>";

					break;
			}

			logger.info(auditby + " Distribution List - " + message);
		...
		if (rowsAffected == -1) {
			...
			url = response.encodeURL("/core/msg.jsp?rtn=dstidx");
		}

		getServletContext().getRequestDispatcher(url).forward(request, response);

		logger.info("--------------------------- DistributionServlet - END");
	}
