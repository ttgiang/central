...
public class TaskCleaningServlet extends HttpServlet {

	private static final long serialVersionUID = 12L;
	static Logger logger = Logger.getLogger(TaskCleaningServlet.class.getName());
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		try {
			if (formName != null && formName.equals("aseForm") && Skew.confirmEncodedValue(request)){
				...
				tsk = website.getRequestParameter(request,"tsk");

				if (TaskDB.recreateTasks(connection,tsk) >= 0){
					message = "Task cleaning completed successfully.<br>";
				}
				else{
					message = "Task cleaning failed.<br>";
				}
