...
public class AssessServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			switch (iAction) {
				case cancelAction:
					...
					break;
				case deleteAction:
					rowsAffected = AssessDB.deleteAssessment(connection, id);
					...
					break;
				case insertAction:
					rowsAffected = AssessDB.insertAssessment(connection, assess);

					if (rowsAffected == 1)
						message = "Assessment inserted successfully";
					else
						message = "Unable to insert assessment";

					break;
				case updateAction:
					rowsAffected = AssessDB.updateAssessment(connection, assess);
		...
		if (rowsAffected == -1) {
			...
			url = response.encodeURL("/core/msg.jsp?rtn=crsassidx");
		}
