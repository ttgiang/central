...
<%
	/**
	*	ASE
	*	crsacany.jsp	- cancel assess process
	*	2007.09.01
	**/
		...
		if ("s".equalsIgnoreCase(formAction) && Skew.confirmEncodedValue(request)){
			...
			*/
			...
			if ( alpha.length() > 0 && num.length() > 0 ){
				msg = SLODB.cancelAssesssment(conn,campus,kix,user);
				if ("Exception".equals(msg.getMsg())){
					message = "Assessment cancellation failed.<br><br>" + msg.getErrorLog();
				}
				else if ( !"".equals(msg.getMsg()) ){
					message = "Unable to cancel assessment.<br><br>" + MsgDB.getMsgDetail(msg.getMsg());
				}
				else{
					message = "Assessment cancelled successfully<br>";
				}
