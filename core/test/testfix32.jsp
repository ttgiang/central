	...
	public static String processItem(Connection conn,String campus){
		...
		try{
			String sql = "SELECT historyid," + fromField
					+ " FROM tblCourse "
					+ " WHERE campus=? AND " + fromField + " is not null "
			...
			while (rs.next()){
				...
				sql = "UPDATE tblCampusData SET " + to + "=? WHERE historyid=?";
				ps2 = conn.prepareStatement(sql);
				ps2.setString(1,fromData);
				ps2.setString(2,historyid);
				rowsAffected = ps2.executeUpdate();
				if (rowsAffected==0){
					logger.info("textfiX32 - " + id + ": " + campus + " - " + historyid);
					++id;
				}
				++i;
			}
			...
			sql = "UPDATE tblCourse SET " + fromField + "='' WHERE campus=?";
			ps2 = conn.prepareStatement(sql);
