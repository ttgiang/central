...
<%
 ...
 public static void processUpdateCounts(int[] updateCounts) {
	  for (int i=0; i<updateCounts.length; i++) {
			if (updateCounts[i] >= 0) {
				 out.println("Successfully executed; the number represents number of affected rows");
			} else if (updateCounts[i] == Statement.SUCCESS_NO_INFO) {
				 out.println("Successfully executed; number of affected rows not available");
			} else if (updateCounts[i] == Statement.EXECUTE_FAILED) {
				 out.println("Failed to execute");
			}
	  }
 }
