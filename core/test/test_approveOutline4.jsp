...
//Msg msg = new Msg();
		...
		try{
			...
			tableCounter = 5;
			sql[0] = "UPDATE tblTempCoreq SET coursetype='ARC' WHERE campus=? AND coursealpha=? AND coursenum=?";
			sql[1] = "UPDATE tblTempPreReq SET coursetype='ARC' WHERE campus=? AND coursealpha=? AND coursenum=?";
			sql[2] = "UPDATE tblTempCourseAssess SET coursetype='ARC' WHERE campus=? AND coursealpha=? AND coursenum=?";
			sql[3] = "UPDATE tblTempCourseComp SET coursetype='ARC' WHERE campus=? AND coursealpha=? AND coursenum=?";
			sql[4] = "UPDATE tblTempCourseContent SET coursetype='ARC' WHERE campus=? AND coursealpha=? AND coursenum=?";
			for (i=0;i<tableCounter;i++){
				//out.println("" + i + ": " + sql[i]+"<br>");
				preparedStatement = conn.prepareStatement(sql[i]);
				preparedStatement.setString(1, campus);
