...
//
//
// ATTENTION: This file has been MODIFIED from the original by Gigaman2003 for
// distribution with the Pure webblog software.
//
// Modifications:
// 1. Modified to accept a percentage width instead of a pixel value.
// 2. Modified with two extra buttons to lengthen and contract the editor.
// 3. Added four custom images for the extra buttons.
//    Images added in icons/up.gif, up_on.gif, down.gif., down_on.gif
// 4. Added three cookie functions from quirksmode.org
//    Functions added: createCookie(), readCookie(), clearCookie()
//
// You can reach me at http://www.halfempty.co.uk (Personal Message) for help
// I will not respond to your emails!
...
//EXTEND_AND_CONTRACT_MOD
// WYSIWYG Width and Height
wysiwygWidth = 100; //This is the default width in PERCENT
wysiwygHeight = 200; //This is the default height in PX
each_extension = 100; //This is in PX, and is how many pixels the editor will be adjusted per click

//This holds the defaults up there
d_width = wysiwygWidth;
d_height = wysiwygHeight;
//END_EXTEND_AND_CONTRACT_MOD
...
//EXTEND_AND_CONTRACT_MOD
  //buttonName2[16]  = "help"; //we put this somewhere else
//END EXTEND_AND_CONTRACT_MOD
...
//Custom textarea altering functions
function addheight_wysiwyg(textareaID,height) {
	//Get the original height
	readHeight=Number(readCookie(textareaID+'height'));

	//Get the new height
	var newheight = readHeight+height;

	// Give the table the new height
	document.getElementById(textareaID+"wysiwygtable").style.height = newheight + "px";
	document.getElementById("wysiwyg"+textareaID).style.height = newheight + "px"; //Reset iframe

	//Update our cookie
	createCookie(textareaID+'height',newheight,9999);
}

function minusheight_wysiwyg(textareaID,height) {
	//Get the original height
	readHeight=Number(readCookie(textareaID+'height'));
	...
	if(newheight<0)
	{
		alert('The textarea is already at it\'s minimum height.');
	}
	else
	{
		// Give the table the new height
...
function recallsize(textareaID) {
	//I iz scared. K i'll come out for cookie.
	// Checks for a height cookie
	if(readCookie(textareaID+'height')!==null)
	{
		try{
			readHeight=Number(readCookie(textareaID+'height'));
		}
		...
		{
			alert('Custom height cookie reading failed: ' + err);
			createCookie(textareaID+'height',wysiwygHeight,9999); //Try and fix the cookie by overwriting it with a new one.
		}
		try{
			document.getElementById(textareaID+"wysiwygtable").style.height = readHeight + "px"; //Reset textarea
			document.getElementById("wysiwyg"+textareaID).style.height = readHeight + "px"; //Reset textarea
		}
		catch(err)
		{
			alert('Custom textbox height change failed: ' + err);
		}
	}
	//If no cookie ;( then we need to set it up anyway
	else
	{
		createCookie(textareaID+'height',wysiwygHeight,9999); //Make new cookie with default
		document.getElementById(textareaID+"wysiwygtable").style.height = wysiwygHeight + "px"; //Reset textarea
			document.getElementById("wysiwyg"+textareaID).style.height = wysiwygHeight + "px"; //Reset textarea
	}
}
//END EXTEND_AND_CONTRACT_MOD
...
function generate_wysiwyg(textareaID) {
  ...
  var n = textareaID;
	...
	// Toolbars width is unchanged because we use a % width
  ...
  for (var j = 0; j <= buttonName2.length;) {
	...
	//EXTEND_AND_CONTRACT_MOD
	//Add on the extend and contract buttons
		toolbar2 += '<td style="width: 22px;"><img src="'+imagesDir+ 'down.gif" border=0 unselectable="on" title="ExtendTextarea" id="ExtendTextarea" class="button" onClick="addheight_wysiwyg(\''+n+'\','+each_extension+');" onmouseover="if(className==\'button\'){className=\'buttonOver\'}; this.src=\''+imagesDir+ 'down_on.gif\';" onmouseout="if(className==\'buttonOver\'){className=\'button\'}; this.src=\''+imagesDir+ 'down.gif\';" unselectable="on" width="20" height="20"></td>';
		toolbar2 += '<td style="width: 22px;"><img src="'+imagesDir+ 'up.gif" border=0 unselectable="on" title="ContractTextarea" id="ContractTextarea" class="button" onClick="minusheight_wysiwyg(\''+n+'\','+each_extension+');" onmouseover="if(className==\'button\'){className=\'buttonOver\'}; this.src=\''+imagesDir+ 'up_on.gif\';" onmouseout="if(className==\'buttonOver\'){className=\'button\'}; this.src=\''+imagesDir+ 'up.gif\';" unselectable="on" width="20" height="20"></td>';
	//Pop in a seperator
		toolbar2 += '<td style="width: 12px;" align="center"><img src="'+imagesDir + 'seperator.gif" border=0 unselectable="on" width="2" height="18" hspace="2" unselectable="on"></td>';
	//Pop in the help button
		    toolbar2 += '<td style="width: 22px;">';
				toolbar2 += '<span id="HTMLMode' + n + '"><img src="'  +imagesDir+  'help.gif" border=0 unselectable="on" title="Help" id="Help" class="button" onClick="formatText(this.id,\'' + n + '\');" onmouseover="if(className==\'button\'){className=\'buttonOver\'}; this.src=\'' +imagesDir+ 'help_on.gif\';" onmouseout="if(className==\'buttonOver\'){className=\'button\'}; this.src=\'' + imagesDir + 'help.gif\';" unselectable="on"  width="20" height="20"></span>';
	      toolbar2 += '</td>';
	//END EXTEND_AND_CONTRACT_MOD
  ...
  toolbar2 += '<td>&nbsp;</td></tr></table>';
	...
	var iframe = '<table id="' + n + 'wysiwygtable" cellpadding="0" cellspacing="0" border="0" style="width:' + wysiwygWidth + '%; height:' + wysiwygHeight + 'px; border: 1px inset #CCCCCC;"><tr><td valign="top">\n'
  + '<iframe frameborder="0" id="wysiwyg' + n + '" width="100%" height="100%"></iframe>\n'
  + '</td></tr></table>\n';
  ...
  if (browserName == "Microsoft Internet Explorer") {
	...
	//EXTEND_AND_CONTRACT_MOD
	//Reset the textbox size to the last customized value if it exists
	recallsize(n);
	//END EXTEND_AND_CONTRACT_MOD
};
