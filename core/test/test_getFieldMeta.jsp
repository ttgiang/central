	...
	**/
	...
	try {
		String sql = "SELECT c.*, "
			...
			+ "FROM tblCourse c, tblCampusData "
			+ "WHERE tblCampusData.historyid=c.historyid AND "
			+ "c.campus=? AND "
			+ "c.coursealpha=? AND "
			+ "c.coursenum=? AND "
			+ "c.coursetype=?";
		...
		if (rs.next()) {
			...
			for (j=0; j<aFields.length; j++) {
				...
				if (question!=null){
					...
					System.out.println(aFields[j] + " - " + thisSeq);
				}
			}

			aseUtil.getRecordToHash(rs, rsHash, aFields);
		}
		rs.close();
		ps.close();
	} catch (SQLException se) {
		System.out.println("CourseFieldsDB: getFieldMeta\n" + se.toString());
	} catch (Exception e) {
		System.out.println("CourseFieldsDB: getFieldMeta\n" + e.toString());
	}

	//out.println(courseDB.enableOutlineItems(conn,campus,alpha,num,user));
 	//out.println(AssessDB.isDeletable(conn,2));
	//out.println(getContentAsHTMLList(conn,alpha,num,campus,type,"64i15c9182",true,true));
	//out.println(showAssessmentsToCancel(conn,campus,user,"");
	//out.println(ContentDB.hasSLO(conn,campus,alpha,num,type,0));
	//out.println(ApproverDB.getDelegateByApproverName(conn,campus,"GOODMANJ"));
	//rowsAffected = TaskDB.logTask(conn,user,"GOODMANJ",alpha,num,"Approve outline",campus,"","REMOVE","");

	//out.println(setCourseForApprovalX(conn,campus,alpha,num,user));
	//logger.info("CourseDB - setCourseForApproval - mail sent - " + nextApprover);
	//out.println(SLODB.isMatch(conn,campus,alpha,num));

	//out.println("*" + SLODB.sloProgress(conn,"c58q18b9942266","INPROGRESS") + "*");
	//ReportDriver.runReport(conn,"C:\\tomcat\\webapps\\central\\temp\\jasper\\sample\\jasperreport2.jrxml");
	//Approver approver = getNextPersonToApprove(conn,campus,alpha,num);
	//out.println(approver.getApprover());
	//out.println(approver.getDelegated());
	//out.println(approver.getSeq());
	//out.println(isNextApprover(conn,campus,alpha,num,"MCABRAL"));
	//out.println(setCourseForApproval(conn,campus,alpha,num,user,""));
	//out.println(viewOutline(conn,"026g5c9432193",Constant.COURSETYPE_CUR));
...
<%!
	...
	public static Msg approveOutlineX(Connection connection,
												...
												boolean approval,
												String comments,
												int voteFor,int voteAgainst,int voteAbstain) throws Exception {
...
Logger logger = Logger.getLogger("test");
		...
		try {
			...
			if (approverSequence > 0) {
				...
				ps.setString(5, user);
				ps.setInt(6, getNextSequenceNumber(connection));
				...
				if (!approval) {
					...
					*/
					...
					if (user.equalsIgnoreCase(approver.getLastApprover()) || user.equalsIgnoreCase(approver.getLastDelegate())) {
						...
						switch(AseUtil.getDriverType(0)){
							...
							case Constant.DATABASE_DRIVER_ACCESSSQL:
								msg = approveOutlineAccess(connection,campus,alpha,num,user);
								break;
						...
						*/
						...
						DeanLevelApproval = IniDB.getIniByCampusCategoryKidKey1(connection,campus,"System","DeanLevelApproval");
						if (Integer.parseInt(DeanLevelApproval)==approverSequence){
							nextApprover = ApproverDB.getApproversBySeq(connection,campus,approverSequence);
						}
						else{
							if (!"".equals(delegated))
					...
					if (msg.getCode() != ERROR_CODE){
						...
						approverSequence = ApproverDB.getApproverSequence(connection,user);
						DeanLevelApproval = IniDB.getIniByCampusCategoryKidKey1(connection,campus,"System","DeanLevelApproval");
						if (Integer.parseInt(DeanLevelApproval)==approverSequence){
							nextApprover = ApproverDB.getApproversBySeq(connection,campus,approverSequence);
							tasks = nextApprover.split(",");
							for (z=0;z<tasks.length;z++){
								rowsAffected = TaskDB.logTask(connection,proposer,tasks[z],alpha,num,"Approve outline",campus,"crsappr.jsp","REMOVE",type);
								logger.info("CourseApproval - approveOutlineX - delete approver task (" + tasks[z] + ") rowsAffected " + rowsAffected);
							}
						}
						else{
							delegated = ApproverDB.getDelegateByApproverName(connection,campus,user);
