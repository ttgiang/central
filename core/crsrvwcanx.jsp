...
<%
	...
	**/
		...
		if ( formName != null && formName.equals("aseForm") ){
			if (formAction.equalsIgnoreCase("s") && Skew.confirmEncodedValue(request)){
				...
				if (progress.equals(Constant.COURSE_REVIEW_TEXT) && (!proposer.equals(user) && !reviewInReview)){
					...
					msg = courseDB.cancelOutlineReview(conn,campus,alpha,num,user,level);
					if ("Exception".equals(msg.getMsg())){
						message = "Outline review cancellation failed.<br><br>" + msg.getErrorLog();
					}
					else if ( !msg.getMsg().equals(Constant.BLANK) ){
						message = "Unable to cancel outline review.<br><br>" + MsgDB.getMsgDetail(msg.getMsg());
					}
					else{
	...
	asePool.freeConnection(conn,"crsrvwcanx",user);
%>
