	...
	**/
		...
		if (processPage){
			...
			if (	(progress.equals(Constant.COURSE_APPROVAL_TEXT) || progress.equals(Constant.COURSE_DELETE_TEXT))
				...
				if (!courseDB.isNextApprover(conn,campus,alpha,num,user)){
					error = true;
					response.sendRedirect("crsappry.jsp?kix=" + kix);
				}
				else{
	...
	if (processPage && !error){
		...
		if (enableVotingButtons.equals(Constant.ON)){
			...
			if (
					(progress.equals(Constant.COURSE_APPROVAL_TEXT) && progress.equals(Constant.COURSE_REVIEW_IN_APPROVAL)) ||
...
<head>
	...
	<script type="text/javascript" src="js/crsappr.js"></script>
...
<%
	if (processPage){
		...
		out.println("<div style=\"position:relative\" id=\"position-relative\">"
			...
			+ "</div>");
%>
	<form method="post" action="crsapprx.jsp" name="aseApprovalForm">
...
<%
	asePool.freeConnection(conn,"crsappr",user);
%>
