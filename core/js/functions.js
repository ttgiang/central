...
/**************************************************************
 SubtractDays: Returns a Date containing a date to which a
          specified days interval has been added to the
          current date.
 ...
 Returns: Date/String
***************************************************************/
function SubtractDays(DaysToAdd)
{
	var newdate = new Date();
	var newtimems = newdate.getTime() - (DaysToAdd * 24 * 60 * 60 * 1000);
	newdate.setTime(newtimems);
...
function asePopUpWindow(mypage,myname,w,h,scroll,pos){
	...
	aseWindow=window.open(mypage,myname,settings);
}

function asePopUpWindowX(mypage,w,h){

	var myname = "aseWin_asePopUpWindowX";
	//var w = "800";
	//var h = "600";
	var scroll = "yes";
	var pos = "center";
...
onunload = function(){
	...
	//var foo = typeof document.forms['form2'] != 'undefined';
	//alert('exists: ' + foo);

	if (	document.aseForm &&
			document.aseForm.catchChange &&
			document.aseForm.catchChange.value == '1')
		useThis = true;

	if (useThis){
		if (isFormDirty(document.forms[0]))
...
function fckeditorWordCount(editorInstance) {
	...
	var wordCountText = "<span class=\"block\">"
							+ "<span class=\"w3c\">Word count: </span>"
							+ "<span class=\"spec\">&nbsp;"+ count + (count == 1 ? "" : "") +"</span>"
							+ "</span>";

	if (document.getElementById('editorWordCountExplain')){
		document.getElementById('editorWordCountExplain').innerHTML = wordCountText;
	}
	else if (document.getElementById('editorWordCount')){
		document.getElementById('editorWordCount').innerHTML = wordCountText;
	}
}
...
/**************************************************************
                               replaceAll
 **************************************************************/
function replaceAll(txt, replaceThis, withThat) {

  return txt.replace(new RegExp(replaceThis, 'g'),withThat);
