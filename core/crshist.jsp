...
<%
	...
	**/
	...
	// run as ant job with the following command line syntax
	// ant -Darg0=LEE testOutlinesWithComments
	...
	if (processPage){
		try{
			...
			//com.test.aseutil.OutlinesWithComments owc = new com.test.aseutil.OutlinesWithComments();
			//out.println("document(s) written: " + owc.createOutlineComments(conn,campus,kix,doHTM,doXML,doPDF,debug));
			//out.println("document(s) written: " + owc.createProgramComments(conn,campus,kix,doHTM,doXML,doPDF,debug,type,showHistory));
			//owc = null;
			out.println("document(s) written: " + createOutlineComments(conn,campus,kix,doHTM,doXML,doPDF,debug,type,showHistory));
		} catch (Exception e){
			...
			if (conn != null){
				...
				try {
					...
					while (rs.next()) {
						...
						if(debug) logger.info("checkedName: " + checkedName);
						...
						if(!file.exists()){
							...
							try{
								...
								if(doHTM){
									...
									if(debug) logger.info("getCourseDescriptionByTypePlus");

									com.ase.aseutil.Msg msg = viewOutline(conn,kix,user,compressed,print,false,detail);
									if(debug) logger.info("viewOutline");
...
FileWriter fstream1 = new FileWriter("C:\\tomcat\\webapps\\centraldocs\\docs\\programs\\ttg\\htm\\CUR\\log1.txt");
BufferedWriter output1 = new BufferedWriter(fstream1);

FileWriter fstream2 = new FileWriter("C:\\tomcat\\webapps\\centraldocs\\docs\\programs\\ttg\\htm\\CUR\\log2.txt");
BufferedWriter output2 = new BufferedWriter(fstream2);
...
output2.close();
	...
	public static int createOutlineCommentsX(Connection conn,String campus) throws Exception {
				...
				try {
					...
					while (rs.next()) {
						...
						try{
							...
							if (fid == 0){
								aphist = "<table width=\"680\" cellspacing=\"1\" cellpadding=\"4\"><tr><td>";
								aphist += "<hr size=1>" + ReviewerDB.getReviewHistory2(conn,kix,0,campus,0,Constant.REVIEW,"h0","c0");
								aphist += "<hr size=1>" + ReviewerDB.getReviewHistory2(conn,kix,0,campus,0,Constant.APPROVAL,"h1","c1");
								aphist += "<hr size=1>" + ReviewerDB.getReviewHistory2(conn,kix,0,campus,0,Constant.REVIEW_IN_APPROVAL,"h2","c2");
								aphist += "</td></tr></table>";
								if(aphist != null){
									aphist = aphist.replace("display: none;","display: nn;");
								}
